...
def loverWindows(lover, loverRole):
    loverWindow = tkinter.Tk()
    loverWindow.geometry("+545+530")
    loverWindow.resizable(height = False, width = False)
    loverWindow.configure(bg = "#c70151")

    loverText = tkinter.Label(loverWindow, text = lover + " is your Lover!\nTheir role is " + loverRole + "\nIf they stay alive, you win!")
    loverText.grid(row = 0, column = 0)
    loverText.configure(bg = "#c70151")
...
def customizing():
    ...
    global customWindow, howManyRetri, retris, howManyPb, plaguebearers, howManyGa, guardianangels, target, roles2, loverModeTrue, loverMode
    ...
    loverTrue = loverModeTrue.get().lower()

    loverCopy = loverTrue

    loverMode = False

    if loverTrue == "y" or loverTrue == "yes":
        loverMode = True
        loverTrue = True

    if loverTrue == "n" or loverTrue == "no":
        loverMode == False
        loverTrue = False

    if loverTrue == loverCopy:
        canGameStart = False
        messagebox.showinfo("Specify", "Please specify: is this Lovers mode or not?")
...
def customGame():
    global roles, custom, howManyJailors, howManyCops, howManyDocs, howManyMafia, howManyJesters, howManyHeirs, customizing, rolesUnlocked, customWindow, howManyRetri, howManyPb, howManyGa, loverModeTrue
    ...
    loverModeTrueLabel = tkinter.Label(customWindow, text = "Is this lovers mode? (y, n)").place(x = 250, y = 200)
    loverModeTrue = tkinter.Entry(customWindow)
    loverModeTrue.place(x = 410,y = 200)
...
def finished():
    global root, nightNumber, dayNumber, coins, winners, wins, winLabel, file, money, game1, target, Alive, playersAndTheirRoles, deadWindow, One, Two, Three, Four, Five, Six, Seven, RealName, PlayerRole, bot1Role, bot2Role, bot3Role, bot4Role, bot5Role, bot6Role, bot7Role, night, Day, deadWindow, jestersLynched, callWinner
...
def died2():
    if mafKilled == RealName or jailorKilled == RealName or pestKilled == RealName or jestHaunt == RealName or mlover == RealName or jlover == RealName or jestlover == RealName or plover == RealName or llover == RealName:
        Day.configure(bg = "cyan")
    else:
        night.configure(bg = "#A9A9A9")

def died():
    global night, Day, mafKilled, lynched, RealName, jailorKilled, pestKilled, jestHaunt, mlover, jlover, jestlover, plover, llover
...
def Days():
    ...
    global lynched, lynchedRole, lynchedTrue, whoWasLynched
    ...
    global gHeal, didgHeal, gHeals
    global lovers, notLovers, Lovers, loverLynched
    global mafCanKill, jailorCanJail, pestCanKill

    mafCanKill = []

    for players in AliveTown:
        mafCanKill.append(players)

    for players in AliveMaf:
        if players in Lovers:
            if lovers[players] in mafCanKill:
                mafCanKill.remove(lovers[players])

    jailorCanJail = []

    for players in Alive:
        jailorCanJail.append(players)
        if playersAndTheirRoles[players] == "Jailor" and players in Lovers:
            jailorCanJail.remove(players)
            if lovers[players] in jailorCanJail:
                jailorCanJail.remove(lovers[players])

    pestCanKill = []

    for player in Alive:
        pestCanKill.append(player)
    for players in plaguebearer:
        pestCanKill.remove(players)
        if players in lovers:
            if lovers[players] in pestCanKill:
                pestCanKill.remove(lovers[players])
    ...
    if (dayNumber > 1 and len(AliveMaf) > 1) or (len(AliveMaf) == 1 and jailed != AliveMaf[0]):
        if RealName not in AliveMaf and len(AliveTown) > 0:
            mafKilled = random.choice(mafCanKill)
            didMafKill = True
    ...
    if RealName in Lovers and dayNumber == 1:
        playerLover = lovers[RealName]
        messagebox.showinfo("Lover!", playerLover + " is your lover. \nTheir role is " + playersAndTheirRoles[playerLover] + "\nIf they stay Alive, you win!")
        loverWindows(playerLover, playersAndTheirRoles[playerLover])
    ...
    if didMafKill == True and mafKilled != RealName and mafKilled != "":
        ...
        if mafKilled in Alive:
            Alive.remove(mafKilled)
        if mafKilled in AliveTown:
            AliveTown.remove(mafKilled)

        if mafKilled in plaguebearer:
            plaguebearer.remove(mafKilled)

        if mafKilled in Lovers:
            Lovers.remove(mafKilled)
    ...
    for player in playersAndTheirRoles:
        if player in Alive and PlayerRole != "Plaguebearer":
            if playersAndTheirRoles[player] == "Pestilence":
                ...
                pestKilled = random.choice(pestCanKill)
    ...
    if dayNumber > 1:
        if didPestKill == True:
            ...
            pestKilledLabel = tkinter.Label(deadWindow, text = "Night: " + str(nightNumber) + "  " + pestKilled + " was killed by Pestilence")
    ...
    if dayNumber > 1:
        if mafKilled in lovers:
            mlover = lovers[mafKilled]
            mlTrue = True
        else:
            mlTrue = False

        if jailorKilled in lovers:
            jlover = lovers[jailorKilled]
            jlTrue = True
        else:
            jlTrue = False

        if jestHaunt in lovers:
            jestlover = lovers[jestHaunt]
            jestlTrue = True
        else:
            jestlTrue = False

        if pestKilled in lovers:
            plover = lovers[pestKilled]
            plTrue = True
        else:
            plTrue = False

        if loverLynched == True:
            llover = lovers[whoWasLynched]



        if mlTrue == True and mlover in Alive:

            if mlover == RealName:
                died()
            
            mloverLabel = tkinter.Label(deadWindow, text = "Night: " + str(nightNumber) + "  " + mlover + " died of a broken heart. His role was " + playersAndTheirRoles[mlover])
            mloverLabel.grid(row = r, column = 0)
            r += 2

            

        if jlTrue == True and jlover in Alive:
            if jlover == RealName:
                died()
                    
            jloverLabel = tkinter.Label(deadWindow, text = "Night: " + str(nightNumber) + "  " + jlover + " died of a broken heart. His role was " + playersAndTheirRoles[jlover])
            jloverLabel.grid(row = r, column = 0)
            r += 2

        if jestlTrue == True and jestlover in Alive:
            if jestlover == RealName:
                died()
            jestloverLabel = tkinter.Label(deadWindow, text = "Night: " + str(nightNumber) + "  " + jestlover + " died of a broken heart. His role was " + playersAndTheirRoles[jestlover])
            jestloverLabel.grid(row = r, column = 0)
            r += 2

        if plTrue == True and plover in Alive:
            if plover == RealName:
                died()
            ploverLabel = tkinter.Label(deadWindow, text = "Night: " + str(nightNumber) + "  " + plover + " died of a broken heart. His role was " + playersAndTheirRoles[plover])
            ploverLabel.grid(row = r, column = 0)
            r += 2
        ...
        if mlTrue == True:
            if mlover in Alive:
                Alive.remove(mlover)
            if mlover in AliveTown:
                AliveTown.remove(mlover)
            if mlover in plaguebearer:
                plaguebearer.remove(mlover)

            if mlover in infected:
                infected.remove(mlover)

            if mlover in Lovers:
                Lovers.remove(mlover)
            if mafKilled in Lovers:
                Lovers.remove(mafKilled)
        ...
        if jlTrue == True:
            if jlover in Alive:
                Alive.remove(jlover)
            if jlover in AliveTown:
                AliveTown.remove(jlover)
            if jlover in plaguebearer:
                plaguebearer.remove(jlover)

            if jlover in infected:
                infected.remove(jlover)
            if jlover in Lovers:
                Lovers.remove(jlover)
            if jailorKilled in Lovers:
                Lovers.remove(jailorKilled)

        if jestlTrue == True:
            if jestlover in Alive:
                Alive.remove(jestlover)
            if jestlover in AliveTown:
                AliveTown.remove(jestlover)
            if jestlover in plaguebearer:
                plaguebearer.remove(jestlover)

            if jestlover in infected:
                infected.remove(jestlover)

            if jestlover in Lovers:
                Lovers.remove(jestlover)
            if jestHaunt in Lovers:
                Lovers.remove(jestHaunt)

        if plTrue == True:
            if plover in Alive:
                Alive.remove(plover)
            if plover in AliveTown:
                AliveTown.remove(plover)
            if plover in plaguebearer:
                plaguebearer.remove(plover)

            if plover in infected:
                infected.remove(plover)

            if plover in Lovers:
                Lovers.remove(plover)
            if pestKilled in Lovers:
                Lovers.remove(pestKilled)

        if loverLynched == True:
            if llover in Alive:
                Alive.remove(llover)
            if llover in AliveTown:
                AliveTown.remove(llover)
            if llover in plaguebearer:
                plaguebearer.remove(llover)

            if llover in infected:
                infected.remove(llover)

            if llover in Lovers:
                Lovers.remove(llover)
            if whoWasLynched in Lovers:
                Lovers.remove(whoWasLynched)


            if llover == RealName:
                died()

            lynchedLoverLabel = tkinter.Label(deadWindow, text = "Night: " + str(nightNumber) + "  " + llover + " died of a broken heart. His role was " + playersAndTheirRoles[llover])
            lynchedLoverLabel.grid(row = r, column = 0)
            r += 2
    ...
    if (len(AliveMaf) < 1 and "Plaguebearer" not in AliveRoles) or (len(Alive) == len(AliveTown)):
        game = False
        callWinner = "Town"
    ...
    if len(Lovers) == len(Alive) and len(Lovers) > 0:
        game = False
        callWinner = "Lovers"
    ...
    elif game == False and callWinner == "Lovers":
        winner = tkinter.Label(deadWindow, text = "Lovers have won the game!")
        winner.grid(row = r, column = 0)
        r += 2
    ...
    if "Jailor" in townRoles and PlayerRole != "Jailor":
        jailed = random.choice(jailorCanJail)
...
def nights():
    ...
    global windowForAlive, gHeals, didgHeal, gHeal, target, callWinner
    global lovers, notLovers, Lovers, loverLynched, whoWasLynched
...
def customStart():
    ...
    global iMafia, iCops, iDocs, Jailor, AliveRoles, target, isThereGa, loverMode, lovers, notLovers, Lovers
    ...
    if loverMode == True:
        possibleLovers = [One, Two, Three, Four, Five, Six, Seven, RealName]
        Lovers = []
        lovers = {}

        for lover in possibleLovers:
            if playersAndTheirRoles[lover] == "Jester" or playersAndTheirRoles[lover] == "Guardian":
                possibleLovers.remove(lover)

        if len(possibleLovers) > 3:

            lover1 = random.choice(possibleLovers)
            possibleLovers.remove(lover1)

            lover2 = random.choice(possibleLovers)
            possibleLovers.remove(lover2)

            lover3 = random.choice(possibleLovers)
            possibleLovers.remove(lover3)

            lover4 = random.choice(possibleLovers)
            possibleLovers.remove(lover4)

            lovers[lover1] = lover2
            lovers[lover2] = lover1

            lovers[lover3] = lover4
            lovers[lover4] = lover3

            for lover in lovers:
                Lovers.append(lover)
        elif len(possibleLovers) > 1:
            lover1 = random.choice(possibleLovers)
            possibleLovers.remove(lover1)
            lover2 = random.choice(possibleLovers)
            possibleLovers.remove(lover2)
        ...
        elif len(possibleLovers) < 2:
            lovers = {}
            notLovers = [One, Two, Three, Four, Five, Six, Seven, RealName]
            Lovers = []
    else:
        lovers = {}
        notLovers = [One, Two, Three, Four, Five, Six, Seven, RealName]
